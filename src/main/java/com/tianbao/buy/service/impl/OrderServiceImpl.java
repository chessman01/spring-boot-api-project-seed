...
public class OrderServiceImpl implements OrderService{
    ...
    public OrderVO build(long courseId) {
        return render(courseId, null, null, 0);
    }

    @Override
    public OrderVO adjust(long courseId, Long cardId, Long couponId, int personTime) {
        return render(courseId, cardId, couponId, personTime);
    }
    ...
    private OrderVO render(long courseId, Long cardId, Long couponId, int personTime) {
    ...
    private OrderVO.PayDetail calRealPay(Map<String, OrderVO.PayDetail> map) {
        ...
        return new OrderVO.PayDetail("实付款", MoneyUtils.unitFormat(2, realPayFee), realPayFee);
    }
    ...
    private Map<String, OrderVO.PayDetail> calFeeDetail(Course course, int personTime, YenCard card, CouponTemplate coupon,
        ...
        totalDetail = new OrderVO.PayDetail("课程总价", MoneyUtils.unitFormat(2, total), total);
        ...
        if (coupon != null) {
            ...
            couponDisCountDetail = new OrderVO.PayDetail("礼券", MoneyUtils.minusUnitFormat(2, couponDiscount), couponDiscount);
        ...
        if (card.getCashAccount() + card.getGiftAccount() >= (total - couponDiscount) * card.getDiscountRate() / 100) {
            ...
            cardDiscountDetail = new OrderVO.PayDetail("瘾卡优惠", MoneyUtils.minusUnitFormat(2, cardDiscount), cardDiscount);
        ...
        if (card.getCashAccount() + card.getGiftAccount() > total - couponDiscount - cardDiscount) {
            ...
            cardPayDetail = new OrderVO.PayDetail("瘾卡支付", MoneyUtils.minusUnitFormat(2, cardPay), cardPay);
